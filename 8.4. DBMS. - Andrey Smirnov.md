# 8.4. Домашнее задание к занятию «Системы хранения данных (СУБД)». - Андрей Смирнов.
...
Содержимое файла `/var/lib/postgresql/data/pg_hba.conf`:

![sshot8_4_1_1](img/8_4_1-1.jpg)


Настройки ролей (результат выполнения команды `select * from pg_authid;`):

![sshot8_4_1_2](img/8_4_1-2.jpg)


1. Для подключения по TCP/IP с адресов 127.0.0.1/32 и ::1/128 используется метод аутентификации trust (доверительная аутентификация), т.е. подключения с localhost (как IPv4, так и IPv6) не требуют пароля и автоматически считаются доверенными.

2. Для подключений с любых других адресов (не 127.0.0.1/32 или ::1/128) требуется аутентификация по паролю, переданному в хешированном виде с использованием алгоритма MD5.

3. Данное утверждение неверно. Помимо пароля там фигурирует имя пользователя. Т.е. формат записи следующий `функция хеширования+хэш(пароль+логин)`. Т.е. в данном случае `md5+MD5(pass+app)`.

4. Для роли `app` все эти поля имеют значение `t`, т.е. `true`, что означает наличие соответствующих прав. Для остальных ролей - `f`, т.е. `false`, что означает отсутствие соответствующих прав.

5. Для изменения метода аутентификации на `reject` я добавил строчку `- POSTGRES_HOST_AUTH_METHOD=reject`, т.е. `docker-compose.yml` приобрел следующий вид:
...
Содержимое файла `/var/lib/postgresql/data/pg_hba.conf` изменилось, вместо `host all all all md5` теперь `host all all all reject`:

![sshot8_4_1_3](img/8_4_1-3.jpg)


Однако настройки ролей остались те же:

![sshot8_4_1_4](img/8_4_1-4.jpg)


Значения полей `rolcanlogin` и `rolpassword` для роли `app` не изменились и подключение проходит, поскольку в `pg_hba.conf` раньше отрабатывает (находится выше) строка выделенная желтым, которая разрешает подключение без пароля с локальных адресов, т.е. в том числе и для нас, так как подключения выполняется из контейнера.  
...
1. Для удалённых подключений не рекомендуется использовать медоты `trust`(подключение без пароля), `password`(пароль при передаче не шифруется) и `md5`(устаревший хеш MD5 уязвим к brute-force).

2. Для удалённых подключений рекомендуется использовать медоты `scram-sha-256`(устойчивый к перебору алгоритм) и `cert` (аутентификация по SSL-сертификатам).
...
Результат подготовительного этапа в первой конcоли:

![sshot8_4_2_1](img/8_4_2-1.jpg)


Результат выполнения команд во второй конcоли:

![sshot8_4_2_2](img/8_4_2-2.jpg)



Мы получаем ошибку `ERROR:  permission denied for table records`, т.е. искомое сообщение: `permission denied for table records`. это логично, поскольку мы отобрали права на чтение у пользователя `reader`.
