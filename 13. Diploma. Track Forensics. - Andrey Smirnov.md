...
06:45:48Z - неудачная попытка входа в учетные записи sshd_server и Wilfred. T1110 (Brute Force).
06:45:48Z - создание учетной записи Support с правами администратора. T1136.001 (Create Account: Local Account).
06:45:48Z - изменение членства в группе Administrators, добавлена учетная запись Support. T1078.001 (Valid Accounts: Local Accounts).
06:45:48Z - изменение членства в группе Users, добавлена учетная запись Support. T1078.001 (Valid Accounts: Local Accounts).
11:32:20Z - успешный вход в учетную запись sshd_server. T1078.001 (Valid Accounts: Local Accounts).
12:23:43Z - успешный вход в учетную запись Support. T1078.001 (Valid Accounts: Local Accounts).
12:30:35Z - успешный вход в учетную запись Wilfred. T1078.001 (Valid Accounts: Local Accounts).
...
На дату 09.03.2019:
14:16:37Z - первоначальная активность (системные утилиты). T1059.003 (Command and Scripting Interpreter: Windows Command Shell). TA0002 (Execution).
14:18:14Z - настройка окружения и автозапуска. T1547.001 (Boot or Logon Autostart Execution: Registry Run Keys / Startup Folder). T1021.002 (Remote Services: SMB/Windows Admin Shares). TA0003 (Persistence).
14:18:25Z - установка Sysinternals BGInfo. T1018 (Remote System Discovery). T1588.002 (Obtain Capabilities: Tool). TA0007 (Discovery).
14:29:31Z - установка 7-Zip (7z1900.exe). T1105 (Ingress Tool Transfer). TA0010 (Exfiltration).
15:35:28Z - работа с файлом passwords.txt. T1555 (Credentials from Password Stores). T1003 (OS Credential Dumping). TA0006 (Credential Access).
16:36:29Z - установка RDP Wrapper (msiexec.exe). T1569.002 (System Services: Service Execution). TA0002 (Execution).
16:50:24Z-16:52:26Z - доступ к SendSpace (file/obeywl, file/0beywl). T1105 (Ingress Tool Transfer). T1048 (Exfiltration Over Alternative Protocol). TA0010 (Exfiltration).
16:57:13Z - работа с реестром (regedit.exe). T1112 (Modify Registry). TA0005 (Defense Evasion).
16:57:35Z - установка персистентности через UserInitMprLogonScript. T1037.005 (Logon Script: Startup). TA0003 (Persistence).
17:34:27Z - RDP подключение к 192.168.1.72. T1021.001 (Remote Desktop Protocol). TA0008 (Lateral Movement).

На дату 10.03.2019 
06:34:02Z - использование Remote Desktop. T1021.001 (Remote Desktop Protocol). TA0008 (Lateral Movement).
06:34:40Z - RDP подключение к 192.168.1.75. T1021.001 (Remote Desktop Protocol). TA0008 (Lateral Movement).
06:35:43Z - работа с сетевыми инструментами (netscan_portable.zip). T1046 (Network Service Scanning). T1018 (Remote System Discovery). TA0007 (Discovery).
06:35:58Z - запуск netscan.exe. T1046 (Network Service Scanning). TA0007 (Discovery).
06:40:02Z - работа с инструментами анализа (iepv.zip). T1588.002 (Obtain Capabilities: Tool). TA0002 (Execution).
06:41:13Z - запуск iepv.exe. T1016 (System Network Configuration Discovery). TA0007 (Discovery).
06:49:46Z - доступ к YouTube и SendSpace. T1071.001 (Application Layer Protocol: Web Protocols). TA0011 (Command and Control).
12:58:26Z - активное использование проводника. T1059.003 (Command and Scripting Interpreter: Windows Command Shell). TA0002 (Execution).
13:03:06Z - запуск RamCapture.exe. T1003 (OS Credential Dumping). T1041 (Exfiltration Over C2 Channel). TA0006 (Credential Access).
13:05:52Z - запуск FTK Imager.exe. T1005 (Data from Local System). T1588.002 (Obtain Capabilities: Tool). TA0009 (Collection).
13:08:15Z - установка AccessData (Forensic Tools). T1588.002 (Obtain Capabilities: Tool). TA0002 (Execution).
...
06:58:23Z - доступ к sendspace.com/file/fn3pal
06:59:18Z - первый запуск 111.bat
07:07:07Z - доступ к sendspace.com/file/vrbg1p
07:08:02Z - запуск 2.bat
12:29:33Z - доступ к sendspace.com/file/qlaheb
12:30:05Z - запуск 3.bat
...
06:45:48Z - создание учетной записи Support. T1136.001 (Create Account: Local Account). TA0003 (Persistence).
06:47:09Z - изменение автозагрузки и настроек автозапуска. T1547.001 (Boot or Logon Autostart Execution: Registry Run Keys). T1021.002 (Remote Services: SMB/Windows Admin Shares). TA0003 (Persistence).
06:47:14Z - установка Sysinternals BGInfo. T1018 (Remote System Discovery). T1588.002 (Obtain Capabilities: Tool). TA0007 (Discovery).
06:58:23Z - доступ к sendspace.com/file/fn3pal. T1105 (Ingress Tool Transfer). T1048 (Exfiltration Over Alternative Protocol). TA0010 (Exfiltration).
06:59:18Z - первый запуск 111.bat. T1059.003 (Command and Scripting Interpreter: Windows Command Shell). TA0002 (Execution).
07:07:07Z - доступ к sendspace.com/file/vrbg1p. T1105 (Ingress Tool Transfer). T1048 (Exfiltration Over Alternative Protocol). TA0010 (Exfiltration).
07:08:02Z - запуск 2.bat. T1059.003 (Command and Scripting Interpreter: Windows Command Shell). TA0002 (Execution).
07:08:47Z - запуск taskmgr.exe. T1059.003 (Command and Scripting Interpreter). T1047 (Windows Management Instrumentation). TA0007 (Discovery).
07:10:34Z - запуск explorer.exe. T1059.003 (Command and Scripting Interpreter). T1204.002 (User Execution: Malicious File). TA0002 (Execution).
07:11:19Z - запуск notepad.exe. T1059.003 (Command and Scripting Interpreter). TA0002 (Execution).
12:23:43Z - вход в систему через учетную запись Support. T1078 (Valid Accounts). TA0005 (Defense Evasion).
12:29:33Z - доступ к sendspace.com/file/qlaheb. T1105 (Ingress Tool Transfer). T1048 (Exfiltration Over Alternative Protocol). TA0010 (Exfiltration).
12:30:05Z - запуск 3.bat и доступ IE к Temp. T1059.003 (Command and Scripting Interpreter). T1204.002 (User Execution: Malicious File). TA0002 (Execution).
12:30:11Z - запуск cmd.exe. T1059.001 (PowerShell). T1059.003 (Windows Command Shell). TA0002 (Execution).
...
На основе проведенного анализа содержимого bat-файлов и `Refund_form.lnk`, можно построить схему взаимодействия с командными срверами: 
...
- Описание: Пользователь вручную запускал подозрительные файлы, включая инструменты разведки и вредоносные скрипты.
- Доказательства:
  - Запуск iepv.exe, netscan.exe, RamCapture.exe
  - Записи в UserAssist о запуске подозрительных приложений
...
Техника: T1048 - Exfiltration Over Alternative Protocol
...
- Доказательства:
  ...
  - Временная корреляция между доступом к SendSpace и запуском инструментов сбора данных
...
6. Проведение обучение персонала по защите от фишинговых атак  
